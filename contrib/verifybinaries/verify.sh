...
###   koricore.org and kori.org and compares them.
...
WORKINGDIR="/tmp/kori_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://koricore.org"
HOST2="https://kori.org"
BASEDIR="/bin/"
VERSIONPREFIX="kori-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kori-',
...
if [ $? -ne 0 ]; then
   echo "kori.org failed to provide signature file, but koricore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "kori.org and koricore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Kori Core binary release signing key installed?"
   fi
