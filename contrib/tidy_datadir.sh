#!/bin/bash
# Copyright (c) 2013 The Kori Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Kori database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Kori datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Kori datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Kori 0.7 datadir."
    ;;
  3)
    echo "Detected Kori pre-0.8 datadir."
    ;;
  4)
    echo "Detected Kori 0.8 datadir."
    ;;
esac
