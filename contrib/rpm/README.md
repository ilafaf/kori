...
The RPM spec file provided here is for Kori-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kori.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Kori git repository](https://github.com/kori/kori) but are not part of
...
Currently only `Source10` is used, for the example `kori.conf` file.
...
in the kori git repository. At present only an SVG version of the Kori
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kori
...
needed when 0.12.1 is released, a proper fix is already in the Kori git
master branch.
...
idea to build Kori Core against a static version of that library compiled
...
The `kori.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kori-server` package. At this point, most RPM based
...
The `kori-server` package should have SELinux support. How to properly do
...
Kori-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kori/). He has been
packaging Kori for Fedora at least since 2012.
...
4. I renamed the `kori` package that contains the Qt GUI to `kori-core` as
that appears to be how the general population refers to it, in contrast to
`kori-xt` or `kori-classic`. I wanted to make sure the general population
